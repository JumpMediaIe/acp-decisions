"""Fetch refusal reasons from Wicklow County Council's iDocs document portal.

Wicklow hosts decisions at wicklowcoco.eplanning.ie/idocswebDPSS — the same
iDocsWebDPSS platform Kildare and Meath use, just on a different subdomain.
The path to a single PDF is a 4-hop chain because the legacy ASP.NET WebForms
app wraps the file in two layers of iframes and requires a session cookie
set by listFiles.
...
BASE = "https://wicklowcoco.eplanning.ie/idocswebDPSS"
USER_AGENT = (
...
def main() -> int:
    ...
    sql = (
        ...
        " WHERE pa.planning_authority = 'Wicklow County Council' "
    ...
    print(f"to fetch: {len(rows):,} Wicklow refusals", flush=True)
    if not rows:
        return 0
