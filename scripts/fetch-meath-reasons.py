"""Fetch refusal reasons from Meath County Council's iDocs document portal.

Meath hosts decisions at idocswebdpss.meathcoco.ie/iDocsWebDPSS — the same
iDocsWebDPSS platform Kildare uses, just on a different subdomain. The path
to a single PDF is a 4-hop chain because the legacy ASP.NET WebForms app
wraps the file in two layers of iframes and requires a session cookie set
by listFiles.
...
BASE = "https://idocswebdpss.meathcoco.ie/iDocsWebDPSS"
USER_AGENT = (
...
def main() -> int:
    ...
    sql = (
        ...
        " WHERE pa.planning_authority = 'Meath County Council' "
    ...
    print(f"to fetch: {len(rows):,} Meath refusals", flush=True)
    if not rows:
        return 0
